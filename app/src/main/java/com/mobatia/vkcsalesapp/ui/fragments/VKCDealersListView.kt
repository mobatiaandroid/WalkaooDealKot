...
import com.mobatia.vkcsalesapp.manager.AppPrefenceManager.saveDealerListCount
...
class VKCDealersListView() : Fragment(), VKCUrlConstants {
    ...
    var imageViewSubmit: ImageView? = null
    ...
    private fun init(v: View?, savedInstanceState: Bundle?) {
        ...
        imageViewSubmit = v.findViewById<View>(R.id.imageViewSubmit) as ImageView
        ...
        imageViewSubmit!!.setOnClickListener(ClickOnView())
    ...
    private fun parseCity(jsonObject: JSONObject): DealersCityModel {
        ...
        Log.v("LOG", "06012015 " + cityModel.cityName)
        val arrayShops: JSONArray
        try {
    ...
    private inner class ClickOnView() : View.OnClickListener {
        override fun onClick(v: View) {
            ...
            if (v === imageViewSubmit) {
                ...
                for (i in AppController.dealersModels.indices) {
                    if (AppController.dealersModels[i].isSelected) {
                        AppController.listDealers
                            .add(AppController.dealersModels[i].id)
                    }
                }
                if (AppController.listDealers.size != 0) {
                    if (AppController.listDealers.size < 7) submitMyDealersApi() else Toast.makeText(
                        activity,
                ...
                override fun responseSuccess(successResponse: String?) {
                    ...
                    try {
                        ...
                        /*
                            ...
                            saveDealerListCount(
                                (activity)!!,
                                AppController.listDealers.size.toString()
                            )
                            AppController.listDealers.clear()
                            /*
								 * mFragmentManager =
