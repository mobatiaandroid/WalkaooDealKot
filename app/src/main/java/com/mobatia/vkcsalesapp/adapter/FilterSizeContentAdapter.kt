 ...
 * @author archana.s
 */
class FilterSizeContentAdapter constructor(
    mContext: Activity,
    filterArrayList: ArrayList<SizeModel>,
    tempArrayList: ArrayList<SizeModel>
) : BaseAdapter() {
    private val mContext: Context
    private val mInflater: LayoutInflater
    private val filterArrayList: ArrayList<SizeModel>
    var tempArrayList: ArrayList<SizeModel>
        ...
        chechBoxType.setOnClickListener(object : View.OnClickListener {
            public override fun onClick(arg0: View) {
                ...
                if (isChecked) {
                    ...
                    AppController.tempsizeCatArrayList.add(filterArrayList.get(position).id!!)
                } else {
                    removeSizeModel(filterArrayList.get(position))
                    AppController.tempsizeCatArrayList.remove(filterArrayList.get(position).id)
                }
            }
        })
    ...
    private fun removeSizeModel(mSizeModel: SizeModel) {
        for (i in tempArrayList.indices) {
            if ((mSizeModel.name == tempArrayList.get(i).name)) {
