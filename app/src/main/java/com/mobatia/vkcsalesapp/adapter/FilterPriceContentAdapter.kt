 ...
 * @author archana s
 */
class FilterPriceContentAdapter constructor(
    mContext: Activity,
    filterArrayList: ArrayList<PriceModel>,
    tempArrayList: ArrayList<PriceModel>
) : BaseAdapter() {
    private val mContext: Context
    private val mInflater: LayoutInflater
    private val filterArrayList: ArrayList<PriceModel>
    var tempArrayList: ArrayList<PriceModel>
    ...
    public override fun getView(position: Int, convertView: View, parent: ViewGroup): View {
        ...
        chechBoxType.setText(
            filterArrayList.get(position).from_range + "-" + filterArrayList.get(position)
                .to_range
        )
        chechBoxType.setOnClickListener(object : View.OnClickListener {
            public override fun onClick(arg0: View) {
                ...
                if (isChecked) {
                    ...
                    AppController.temppriceCatArrayList.add(filterArrayList.get(position).id!!)
                } else {
                    removeBrandModel(filterArrayList.get(position))
                    AppController.temppriceCatArrayList.remove(
    ...
    private fun removeBrandModel(mBrandTypeModel: PriceModel) {
        for (i in tempArrayList.indices) {
            if ((mBrandTypeModel.from_range == tempArrayList.get(i).from_range)) {
